...
(function($) {
	...
	ApImageZoom.prototype = {
		...
		_init: function() {
			...
			if (!this.imageUrl) {
				...
				this.$image
					...
					.error(function() {
						self.$image.attr('src', '/image/viewer/error.png');
					})
		...
		_setup: function() {
			...
			initZoom = this._parseZoom(this.settings.minZoom);
		...
		_onClick: function(evt) {
			...
			this.preventClickIfDoubleClick = false;
			// if (evt.type == 'tap') {
				if (jQuery('#viewer-container').css('visibility') == 'visible') {
					old.viewer.showNextSlide();
				} else if (jQuery('#viewer-topic-container').css('visibility') == 'visible') {
					lm.ui.viewerTopic.showNextSlide();
					if (jQuery('#viewer-topic-share-menu').css('visibility') == 'visible') {
						jQuery('#viewer-topic-share-menu').css('visibility', 'hidden');
					}
				} else if (jQuery('.js-viewer-tablet-container').css('visibility') == 'visible') { //для tablet-viewer - переписать как и для desktop-viewer
					this.timerForClick = setTimeout(function() {
						if (!this.preventClickIfDoubleClick) {
							if (jQuery('.js-viewer-tablet-panel-share').css('visibility') == 'visible') {
								jQuery('.js-viewer-tablet-panel-share').css('visibility', 'hidden');
							}
						}
					this.preventClickIfDoubleClick = false;
					}, 200);
		...
		_onDoubleClick: function(evt) {
			...
			clearTimeout(this.timerForClick);
			this.preventClickIfDoubleClick = true;
		...
		_zoomTo: function(zoom, origin) {
			...
			if (this.currentZoom == 1) {
				$('#viewer-btn-slide-zoom').addClass('active');
			} else if (this.currentZoom == initZoom) {
				$('#viewer-btn-slide-zoom').removeClass('active');
			}
		},
		...
		_getValidPosition: function(position) {
			...
			if (this.currentSize.width <= overlaySize.width) {
				if ($.inArray(this.settings.autoCenter, [true, 'both', 'x', 'h' , 'horizontal']) > -1) {
					...
					if (windowWitdh % 2) {
						left = -1 * Math.ceil(this.currentSize.width / 2);
					} else {
						left = -1 * this.currentSize.width / 2;
					}
				}
				else {
					left = Math.min(Math.max(0 - Math.ceil(overlaySize.width / 2), position.x), Math.ceil(overlaySize.width / 2) - this.currentSize.width);
				}
			}
			else {
				left = Math.max(Math.min(0 - Math.ceil(overlaySize.width / 2), position.x), Math.ceil(overlaySize.width / 2) - this.currentSize.width);
			}
			...
			if (this.currentSize.height <= overlaySize.height) {
				if ($.inArray(this.settings.autoCenter, [true, 'both', 'y', 'v' , 'vertical']) > -1) {
					top = -1 * Math.ceil(this.currentSize.height / 2);
				}
				else {
					top = Math.min(Math.max(0 - Math.ceil(overlaySize.height / 2), position.y), Math.ceil(overlaySize.height / 2) - this.currentSize.height);
				}
			}
			else {
				top = Math.max(Math.min(0 - Math.ceil(overlaySize.height / 2), position.y), Math.ceil(overlaySize.height / 2) - this.currentSize.height);
			}
			top = Math.round(top);
	...
	ApImageZoom.defaultSettings = {
		...
		loadingAnimationFadeOutDuration: undefined,	// Options: value (float), false/null/undefined
